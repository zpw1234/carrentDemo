    ...
    --%>
        ...
        <script src="${sxt}/static/layui_ext/dist/dtree.js"></script>
        ...
        //alert(params);
        tableIns.reload({
        url: "${sxt}/menu/loadAllMenu?" + params,
        page:{curr:1}
        })
        });
        ...
        if (layEvent === 'del') { //删除
        //判断当前菜单有没有子节点
        $.post("${sxt}/menu/checkMenuHasChildren?id=" + data.id, function (obj) {
        if (obj.code >= 0) {
        layer.msg("当前菜单有子节点，请先删除子节点");
        } else {
        layer.confirm('真的删除【' + data.title + '】这个菜单么？', function (index) {
        //向服务端发送删除指令
        $.post("${sxt}/menu/deleteMenu", {id: data.id}, function (res) {
        layer.msg(res.msg);
        //刷新数据表格
        tableIns.reload();
        //刷新左边的数
        ...
        url: "${sxt}/menu/loadMenuManagerLeftTreeJson?spread=1",  // 使用url加载（可与data加载同时存在）
        icon: "2",
        accordion: true //能否下拉
        });
        ...
        tableIns.reload({
        url: "${sxt}/menu/loadAllMenu?id=" + id
        })
        }
        </script>
        </body>
