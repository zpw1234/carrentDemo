...
<head>
    ...
    <link rel="stylesheet" href="${sxt}/static/css/public.css" media="all"/>
</head>
...
<script src="${sxt}/static/layui/layui.js"></script>
            ...
            , cols: [[   //列表数据
                ...
                , {
                    field: 'carimg', title: '缩略图', align: 'center', width: '80', templet: function (d) {
                        return "<img width=40 height=40 src=${sxt}/file/downloadShowFile?path=" + d.carimg + "/>";
                    }
                }
...
//            alert(params);
        ...
        table.on('tool(carTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('真的删除【' + data.carnumber + '】这个车辆么？', function (index) {
                    //向服务端发送删除指令
                    $.post("${sxt}/car/deleteCar", {carnumber: data.carnumber}, function (res) {
                        layer.msg(res.msg);
                        //刷新数据表格
                        tableIns.reload();
                    })
        ...
        function openAddCar() {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    ...
                    $("#dataFrm")[0].reset();
                    //设置默认图片
                    $("#showCarImg").attr("src", "${sxt}/file/downloadShowFile?path=images/userface2.jpg");
                    $("#carimg").val("images/userface2.jpg");
                    url = "${sxt}/car/addCar";
        ...
        function openUpdateCar(data) {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    ...
                    $("#showCarImg").attr("src", "${sxt}/file/downloadShowFile?path=" + data.carimg);
                    url = "${sxt}/car/updateCar";
        ...
        function deleteBatch() {
            ...
            layer.confirm('真的要删除这些车辆么？', function (index) {
                //向服务端发送删除指令
                $.post("${sxt}/car/deleteBatchCar", params, function (res) {
                    layer.msg(res.msg);
                    //刷新数据表格
                    tableIns.reload();
                })
        ...
        upload.render({
            elem: '#carimgDiv',
            url: '${sxt}/file/uploadFile',
            method: "post",  //此处是为了演示之用，实际使用中请将此删除，默认用post方式提交
            acceptMime: 'images/*',
            field: "mf",
            done: function (res, index, upload) {
                $('#showCarImg').attr('src', "${sxt}/file/downloadShowFile?path=" + res.data.src);
        ...
        function showCarImage(data) {
            mainIndex = layer.open({
                ...
                area: ['750px', '500px'],
                success: function (index) {
                    $("#view_carimg").attr("src","${sxt}/file/downloadShowFile?path="+data.carimg);
                }
            });
        }
